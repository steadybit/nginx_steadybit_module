...
#
# Test script for ngx_http_sleep_module.c
...
echo "=== Testing ngx_http_sleep_module locally ==="
...
cp $ORIGINAL_DIR/ngx_http_sleep_module.c $TEST_DIR/

# Create module config
echo "Creating module config..."
mkdir -p $TEST_DIR/sleep
cp $TEST_DIR/ngx_http_sleep_module.c $TEST_DIR/sleep/
cat > $TEST_DIR/sleep/config << 'EOF'
ngx_addon_name=ngx_http_sleep_module
if test -n "$dynamic_modules" || test -n "$ngx_module_link"; then
    ngx_module_type=HTTP
    ngx_module_name=ngx_http_sleep_module
    ngx_module_srcs="$ngx_addon_dir/ngx_http_sleep_module.c"
    ngx_module_libs=
    . auto/module
else
    HTTP_MODULES="$HTTP_MODULES ngx_http_sleep_module"
    NGX_ADDON_SRCS="$NGX_ADDON_SRCS $ngx_addon_dir/ngx_http_sleep_module.c"
fi
EOF
...
cp $TEST_DIR/nginx-$NGINX_VERSION/objs/ngx_http_sleep_module.so $TEST_DIR/nginx/modules/

# Check if module was actually built
echo "Checking if module was built..."
if [ ! -f "$TEST_DIR/nginx/modules/ngx_http_sleep_module.so" ]; then
    echo "❌ ERROR: Module file not found at $TEST_DIR/nginx/modules/ngx_http_sleep_module.so"
...
file $TEST_DIR/nginx/modules/ngx_http_sleep_module.so
...
nm -D $TEST_DIR/nginx/modules/ngx_http_sleep_module.so 2>/dev/null | grep -i sleep || \
nm $TEST_DIR/nginx/modules/ngx_http_sleep_module.so 2>/dev/null | grep -i sleep || \
...
load_module $TEST_DIR/nginx/modules/ngx_http_sleep_module.so;
